...
var Picker = {
    ...
    methods: {
      ...
      setData  : function(data, selectedIndex) {
        ...
        this.pickerSelectedIndex = selectedIndex ? (Array.prototype.push.apply([],selectedIndex)) : []
          ...
          if (i >= fromColumn) {
            ...
            data.forEach(function(item){
              columnData.push({
                ...
                text: item[vm.textKey]
              })
            })
            vm.pickerData[i] = columnData
            /* refillColumn could only be called after show() */
            vm.pickerSelectedIndex[i] = fromColumn === 0
              ? (vm.pickerSelectedIndex[i] < data.length ? vm.pickerSelectedIndex[i] || 0 : 0)
              : vm.$refs.picker.refillColumn(i, columnData)
          }
          data = data.length ? data[vm.pickerSelectedIndex[i]].children : null

          i++
        }

        vm.pickerData = vm.pickerData.slice()
      }
    },
    components: {
      Picker
...
// date-picker
function range(n, m, polyfill, unit ) {
    polyfill = polyfill || false
    unit = unit || ''
    var arr = [];
    for (var i = n; i <= m; i++) {
...
var DatePicker = {
    template : '<cube-cascade-picker ref="cascadePicker" title="Date Picker" :data="data" :selectedIndex="selectedIndex" @select="select" @cancel="cancel">'
                +'</cube-cascade-picker>',
    name: 'date-picker',
    props: {
        min: {
    ...
    computed: {
        data : function() {
            var vm = this;
            var data = range(vm.min[0], vm.max[0], false, '年')

            data.forEach(function(year){
                var minMonth = year.value === vm.min[0] ? vm.min[1] : 1
                var maxMonth = year.value === vm.max[0] ? vm.max[1] : 12
                ...
                year.children.forEach(function(month){
                    ...
                    var minDay = year.value === vm.min[0] && month.value === vm.min[1] ? vm.min[2] : 1
                    var maxDay = year.value === vm.max[0] && month.value === vm.max[1] ? vm.max[2] : day
    ...
    methods: {
        ...
        select : function(selectedVal, selectedIndex, selectedText) {
            this.$emit('select', selectedVal, selectedIndex, selectedText)
        },
        cancel : function() {
            this.$emit('cancel')
