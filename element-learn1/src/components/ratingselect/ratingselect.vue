<template>
    <div class="ratingselect">
        <div class="rating-type border-1px">
            <div class="block positive" :class="{'active':selectType.value===2}"
                @click="select(2, $event)">
                <span class="text">{{desc.all}}</span>
                <span class="count">{{ratings.length}}</span>
            </div>
            <div class="block positive" :class="{'active':selectType.value===0}"
                @click="select(0, $event)">
                <span class="text">{{desc.positive}}</span>
                <span class="count">{{positives.length}}</span>
            </div>
            <div class="block negative" :class="{'active':selectType.value===1}"
                @click="select(1, $event)">
                <span class="text">{{desc.negative}}</span>
                <span class="count">{{negatives.length}}</span>
            </div>
        </div>
        <div class="switch" :class="{'on':onlyContent.value}"
            @click="toggleContent">
...
export default {
    ...
    methods: {
        ...
        select(type, event) {
            ...
            this.$dispatch('betterScrollRefresh');
        },
        toggleContent(event) {
            if (!event._constructed) {
                return;
